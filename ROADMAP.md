...
graph TD
    ...
    A --> C["🦁 Mascota Exclusiva:### 🪓 Módulo 1: Nuevo Héroe — El Bárbaro (*The Barbarian*)
*Nueva clase de combate cuerpo a cuerpo agresiva centrada en la furia berserker y resistencia al dolor.*
- [x] **Mecánica Única de Clase**: *Medidor de Furia / Rabia* (acumula daño recibido para potenciar ataques devastadores y velocidad).
- [x] **Equipamiento Inicial**: Gran Hacha tosca, pieles curtidas, cuerno de guerra ancestral.
- [x] **Árbol de Talentos (Tiers 1 al 4)**: Bonificaciones a supervivencia a baja vida, ruptura de armadura y torbellino de golpes.
- [x] **Subclases (Nivel 12+)**:
  - *Belicista (Warmonger)*: Desenfreno en combate que ignora efectos de aturdimiento y aumenta el daño crítico a menor salud.
  - *Maestro de Bestias (Beastmaster)*: Sinergia especial potenciada con las mascotas y compañeros.
- [x] **Sprites y Avatares**: Animaciones en pixel art estándar y HD 2x.
...
- [x] **Origen y Hallazgo**: Huevo de Mantícora Legendario con brillo carmesí y púrpura.
- [x] **Habilidades Combinadas Dragón + Araña**:
...
- [x] **Evolución Triple**: Cría de Mantícora $\rightarrow$ Mantícora Joven $\rightarrow$ Mantícora Imperial Alfa.
- [x] **Audio y Efectos Visuales**: Rugido felino-reptil exclusivo y partículas de fuego y veneno.
...
- [x] **Canales de Validación Soportados**:
...
- [x] **Interfaz de Selección Bloqueada**:
...
- [x] **Nueva Sección en Pantalla de Créditos (`AboutScene`)**:
...
- [x] **Sincronización Dinámica / JSON**:
...
| :---: | :--- | :---: | :---: | :---: |
| 1️⃣ | **Sistema de Licenciamiento y Acceso Supporter / Premium** | `services.platform` / Core | Alta | ✅ Completado |
| 2️⃣ | **Mascota Legendaria: La Mantícora (Manticore)** | `actors.mobs.pets` | Alta | ✅ Completado |
| 3️⃣ | **Nuevo Héroe: El Bárbaro (Barbarian)** | `actors.hero` / Sprites | Muy Alta | ✅ Completado |
| 4️⃣ | **Muro de Honor y Créditos de Patrocinadores (Patreon)** | `scenes.AboutScene` / Web | Media | ✅ Completado |
...
- [x] **🚨 ALTA PRIORIDAD: Ausencia de Colisión para Mascotas (Completado):** Desactivar el *blocking* sólido entre el héroe y su mascota, permitiendo a los jugadores intercambiar posición de manera táctica sin quedar atascados en pasillos estrechos.
- [x] **Panel Táctico Rápido (HUD) (Completado):** Una barra minimalista persistente en la pantalla de juego (`PetTacticalPanel`) que permite asignar órdenes (Seguir, Alimentar, Info) directamente desde el HUD de juego.
