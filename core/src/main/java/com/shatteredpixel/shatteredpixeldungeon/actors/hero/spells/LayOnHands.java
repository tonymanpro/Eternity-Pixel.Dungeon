...
public class LayOnHands extends TargetedClericSpell {
	...
	private void affectChar(Hero hero, Char ch){
		long totalHeal = 10 + 5*hero.pointsInTalent(Talent.LAY_ON_HANDS);
		long totalBarrier = 0;
		if (ch == hero){
			...
			Buff.affect(ch, Barrier.class).incShield((int)totalBarrier);
			ch.sprite.showStatusWithIcon( CharSprite.POSITIVE, Long.toString(totalBarrier), FloatingText.SHIELDING );
		} else {
			if (ch.HT - ch.HP < totalHeal){
				totalBarrier = totalHeal - (ch.HT - ch.HP);
				if (ch.HP != ch.HT) {
					ch.HP = ch.HT;
					ch.sprite.showStatusWithIcon(CharSprite.POSITIVE, Long.toString(totalHeal - totalBarrier), FloatingText.HEALING);
				}
				if (totalBarrier > 0) {
					Barrier barrier = Buff.affect(ch, Barrier.class);
					totalBarrier = Math.min(3 * totalHeal - barrier.shielding(), totalBarrier);
					totalBarrier = Math.max(0, totalBarrier);
					if (totalBarrier > 0) {
						barrier.incShield((int)totalBarrier);
						ch.sprite.showStatusWithIcon(CharSprite.POSITIVE, Long.toString(totalBarrier), FloatingText.SHIELDING);
					}
				}
			} else {
				ch.HP = ch.HP + totalHeal;
				ch.sprite.showStatusWithIcon( CharSprite.POSITIVE, Long.toString(totalHeal), FloatingText.HEALING );
			}
		}
	}
}
