...
public class BlessSpell extends TargetedClericSpell {
	...
	private void affectChar(Hero hero, Char ch){
		...
		if (ch == hero){
			...
			long totalHeal = 5 + 5*hero.pointsInTalent(Talent.BLESS);
			if (ch.HT - ch.HP < totalHeal){
				long barrier = totalHeal - (ch.HT - ch.HP);
				barrier = Math.max(barrier, 0);
				if (ch.HP != ch.HT) {
					ch.HP = ch.HT;
					ch.sprite.showStatusWithIcon(CharSprite.POSITIVE, Long.toString(totalHeal - barrier), FloatingText.HEALING);
				}
				if (barrier > 0) {
					Buff.affect(ch, Barrier.class).setShield((int)barrier);
