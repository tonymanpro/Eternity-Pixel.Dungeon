...
import com.shatteredpixel.shatteredpixeldungeon.items.armor.glyphs.Frostbite;
...
public class Hero extends Char {
	...
	public float getMobMultiplier() {
		return 2f * StaircaseTotem.additionalMobMulti();
	}
	...
	public void updateHT( boolean boostHP ){
		...
		double extender = HealthModule.multi;
		if (extender >= 1) {
			HT += (long) (25 * extender);
		}
...
if (buff(RoundShield.GuardTracker.class) != null){
	...
	public String defenseVerb() {
		...
		if (parry != null){
			parry.parried = true;
			if (buff(Combo.class).getComboCount() < 9 || subClass != HeroSubClass.GLADIATOR){
				parry.detach();
			}
	...
	public long drRoll() {
		...
		if (buff(HoldFast.class) != null){
			dr += Dungeon.NormalLongRange(HoldFast.minArmor(), HoldFast.armor());
		}
...
if (!Dungeon.level.visited[cell] && !Dungeon.level.mapped[cell]
	...
	public boolean isImmune(Class effect) {
        ...
        if ((effect == Frost.class || effect == Chill.class)
                && belongings.armor() != null
                && belongings.armor().hasGlyph(Frostbite.class, this)){
            return true;
        }
