	...
	private void affectChar( Char ch ){
		if (ch.alignment == Char.Alignment.ALLY){
			...
			if (ch == Dungeon.hero || ch.HP == ch.HT){
				long barrierToGive = Math.min(15, 30 - ch.shielding());
				Buff.affect(ch, Barrier.class).incShield((int)barrierToGive);
				ch.sprite.showStatusWithIcon( CharSprite.POSITIVE, Long.toString(barrierToGive), FloatingText.SHIELDING );
			} else {
				long barrier = 15 - (ch.HT - ch.HP);
				barrier = Math.max(barrier, 0);
				ch.HP += 15 - barrier;
				ch.sprite.showStatusWithIcon( CharSprite.POSITIVE, Long.toString(15-barrier), FloatingText.HEALING );
				if (barrier > 0){
					Buff.affect(ch, Barrier.class).incShield((int)barrier);
					ch.sprite.showStatusWithIcon( CharSprite.POSITIVE, Long.toString(barrier), FloatingText.SHIELDING );
				}
			}
		} else if (!ch.flying) {
