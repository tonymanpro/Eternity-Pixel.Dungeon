			...
			if (spiritForm != null){
				RedButton btnSpirit = new RedButton(Messages.get(WndUseTrinity.class, "spirit",
					...
					public void onClick() {
						...
						for (Ring r : Dungeon.hero.belongings.rings) {
							if (r != null && r.getClass().equals(spiritForm.getClass())) alreadyEquipped = true;
						}
						for (Artifact a : Dungeon.hero.belongings.artifacts) {
							if (a != null && a.getClass().equals(spiritForm.getClass())) alreadyEquipped = true;
						}
						for (KindofMisc m : Dungeon.hero.belongings.miscs) {
							if (m != null && m.getClass().equals(spiritForm.getClass())) alreadyEquipped = true;
						}
						if (alreadyEquipped){
							GLog.w(Messages.get(Trinity.class, "no_duplicate"));
							hide();
