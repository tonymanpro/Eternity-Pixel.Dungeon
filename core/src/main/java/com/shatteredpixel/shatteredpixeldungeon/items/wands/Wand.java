...
public abstract class Wand extends Item {
	...
	public String info() {
		...
		if (Dungeon.hero != null && Dungeon.hero.isSubclass(HeroSubClass.BATTLEMAGE)){
...
//			}
			...
			if (charger.target.buff(ScrollEmpower.class) != null){
				lvl += Math.max(1, ((Hero)charger.target).lvl / 5);
			}
			...
			if (target != null) {
				...
				if (target == curUser.pos || cell == curUser.pos) {
					if (target == curUser.pos && curUser.heroClass == HeroClass.MAGE){
						...
						float shield = curUser.HT * (0.105f*curWand.curCharges);
