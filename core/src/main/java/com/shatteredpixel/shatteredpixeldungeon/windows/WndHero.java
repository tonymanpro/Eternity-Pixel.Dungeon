...
public class WndHero extends WndTabbed {
	...
	private class StatsTab extends Group {
		...
		public void initialize(){
            ...
            else if (!Hero.customHeroName.isEmpty())
                title.label( Messages.get(this, "title_2", hero.lvl, hero.name() ).toUpperCase( Locale.ENGLISH ) );
			else
			...
			if (Dungeon.weekly) {
                if (!Dungeon.weeklyReplay) {
                    statSlot("_Weekly For_", "_" + Dungeon.customSeedText + "_");
                } else {
                    statSlot("_Weekly Replay For_", "_" + Dungeon.customSeedText + "_");
                }
