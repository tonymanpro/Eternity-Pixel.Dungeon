...
### 1. Control Estricto de Commits y Submódulos
...
- **Sincronización Previa de Submódulos**: Si existen cambios en `core/src/main/assets/packages/eternity/`, commitear y pushear primero el submódulo hacia `Eternity-Commercial-Assets`, y luego subir el repositorio principal con `git push --recurse-submodules=on-demand origin main`.
...
2. **Sincronización y Push de Submódulos**:
   - Verificar si hay cambios en `core/src/main/assets/packages/eternity` y subirlos a `Eternity-Commercial-Assets`.
...
4. **Generación del Paquete de Distribución**:
   ...
   - Artefacto APK Android: `android/build/outputs/apk/release/android-release.apk`
5. **Publicación y Tagging**:
   - Subir el commit a la rama principal: `git push --recurse-submodules=on-demand origin main`
   - Crear y subir el Git Tag: `git tag -fa v<VERSION> -m "Eternity Pixel Dungeon v<VERSION> - <Resumen>"` y `git push origin v<VERSION>`
6. **Documentación del Package / Release (Orientada al Jugador)**:
